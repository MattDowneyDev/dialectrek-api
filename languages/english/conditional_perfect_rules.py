"""
English Conditional Perfect -- Display Rules
------------------------------------------------------------------
"would have" + the past participle (participle_rules), unchanged
across all six persons -- just like the plain conditional, "would"
never varies by person here either.
"""
...
def conjugate_conditional_perfect(infinitive_english: str, pronoun_index: int) -> str:
    """Naive conditional-perfect conjugation of an English gloss, used
    only for display. Picks the first "/"-separated translation; only
    the verb's first word takes the participle form, since any words
    after it (phrasal particles like "out"/"up"/"for") never change.
